...
class MakerDaoJdbcClientDao(
    ...
    override fun findOtherCandidates(memberId: Long): OtherMatchCandidates {
        ...
        val relationMap = memberRelations.associateBy(MemberRelationDto::inviteeId, MemberRelationDto::relationType)
        val matchCandidates =
            matchCandidates(memberRelations.map(MemberRelationDto::inviteeId)).filter(MatchCandidateDto::hasProfile)
        val matchCandidateMap = matchCandidates.associateBy(MatchCandidateDto::id)
        val matchCandidateIds = matchCandidateMap.keys
        val matchCandidateImages = matchCandidateImages(matchCandidateIds)
        val matchCandidateImageMap = matchCandidateImages.groupBy(MatchCandidateImageDto::matchCandidateId)
        val members = members(matchCandidates.map(MatchCandidateDto::memberId))
        ...
        matchCandidates.map { matchCandidate ->
            OtherMatchCandidate(
                ...
                images = matchCandidateImageMap[matchCandidate.id]?.map { CandidateImage(it.imageUrl, it.profileOrder) }
                    ?: emptyList(),
                name = memberMap[matchCandidate.memberId]!!.name,
        ...
        }.let { return OtherMatchCandidates(it) }
    }

    private fun mapToMbtiList(
        matchCandidate: MatchCandidateDto,
    ): List<String> {
        val idealMbti = matchCandidate?.idealMbti!!
    ...
    private fun mapToString(
        ...
        val idealAgeRange = matchCandidate?.idealAgeRange!!
    ...
    private fun mapToList(
        ...
        val idealHeightRange = matchCandidate?.idealHeightRange!!
