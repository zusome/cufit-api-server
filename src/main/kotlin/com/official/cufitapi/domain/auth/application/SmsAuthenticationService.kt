...
class SmsAuthenticationService(
    ...
    override fun issue(command: IssueSmsAuthenticationCommand): SmsAuthentication {
        ...
        CompletableFuture.runAsync {
            smsSender.send(
                command.phone,
                "[큐핏] 인증번호는 ${smsAuthentication.code}입니다. (타인 노출 주의)"
            )
        }
    ...
    override fun verify(command: VerifySmsAuthenticationCodeCommand): SmsAuthentication {
        val smsAuthentication = smsAuthenticationRepository.findAll(command.memberId)
            .sortedByDescending(SmsAuthentication::id)
            .firstOrNull()
